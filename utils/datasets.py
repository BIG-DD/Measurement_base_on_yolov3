...
class load_images_and_labels():
	def __init__(self, path, batch_size=1, img_size=416, augment=False):
		...
		self.label_files = [path.replace('images', 'labelTxt').replace('.png', '.txt').replace('.jpg', '.txt') for path in
							self.img_files]
	...
	def __next__(self):
		...
		for index, files_index in enumerate(range(ia, ib)):
			...
			label_path = self.label_files[self.shuffled_vector[files_index]];#print(label_path)
			...
			# resize成[height height 3]，但是要保证原来的长宽比例，所以对其他的地方用127.5的像素进行填充
			#Load labels
			if os.path.isfile(label_path):
				...
				cls_name = np.where(labels0[:, 0]=='0',0,1) 
				labels[:, 0] = cls_name
